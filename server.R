# Charge les bibliotheques necessaires � l'execution du script
# Elles doivent avoir ete installees auparavant
library(msm)
library(lubridate)
library(MASS)
library(EasyABC)
...
function(input, output, session) {
  alt.shad.spawn<-function(N, 
    ...
    ech<-read.csv(input$file1$datapath,header = input$header,sep = input$sep, dec=input$dec,quote = input$quote)
    time<-1:nrow(ech)
    temperature<-ech[,3]
...
### Plot the distributions & relations corresponding to user's input parameters
  ...
  observeEvent(input$mouline,{
    ...
    abc.lenormand<-ABC_sequential(method="Lenormand",model=alt.shad.spawn.4ABC,prior=prior.abc,nb_simul=500,summary_stat_target = summary.stats.abc)
